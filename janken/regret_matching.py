...
class Player:
    ...
    strategy : np.ndarray
        The mixed strategy that the player has. If it's None, it is set to a uniformly random one.
    Attributes
    ----------
    ...
    def __init__(self, player: int, strategy=None):
    ...
    def simulate_game(self, other_players):
        ...
        if not np.array([isinstance(player, Player) for player in other_players]).all():
            raise NotPlayerError("The inputs are invalid. The acceptable type is only Player.")
        ...
        print('mixed_strategies: ', mixed_strategies)
